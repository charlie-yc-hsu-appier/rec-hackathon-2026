...
I would like to get campaign_ids with group "vendor_inl_corp" when site_id="${site_id}" and service_type_id="${service_type_id}"
...
Get vendor_inl_corp campaign ids from config api
  [Documentation]    Utility keyword to get vendor_inl_corp campaign IDs for Coupang mobile site
  [Arguments]        ${site_id}=android--com.coupang.mobile_s2s_v3    ${service_type_id}=crossx_recommend
  
  I have a config_api session
  ${campaign_ids} =       I would like to get campaign_ids with group "vendor_inl_corp" when site_id="${site_id}" and service_type_id="${service_type_id}"
  Set Test Variable       ${extracted_vendor_inl_corp_campaigns}    ${campaign_ids}
  RETURN                  ${campaign_ids}


I would like to check campaign status when campaign_id="${campaign_id}"
...
Get active vendor_inl_corp campaign ids from config api
  ...
  ${all_campaign_ids} =   I would like to get campaign_ids with group "vendor_inl_corp" when site_id="${site_id}" and service_type_id="${service_type_id}"
  ...
  FOR  ${campaign_id}  IN  @{all_campaign_ids}
    IF  '${campaign_id}' != '' and '${campaign_id}' != '${EMPTY}'
      ${campaign_info} =      I would like to check campaign status when campaign_id="${campaign_id}"
